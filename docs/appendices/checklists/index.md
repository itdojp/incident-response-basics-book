...
|---|---|---|---|
| 10:00 | アラート: 決済 API の 5xx 率が上昇 | IC 任命、Severity 暫定 P1 | incident-log / timeline |
| 10:05 | 影響確認: 失敗率 6%（直近 30 分） | 15 分ごとの状況共有を開始 | status-update |
| 10:10 | 直近変更: deploy#123 が疑わしい | ロールバック可否と副作用（DB 変更有無）を確認 | incident-log |
| 10:15 | DB マイグレーションなし | ロールバック判断（手順/検証項目を確認） | timeline |
